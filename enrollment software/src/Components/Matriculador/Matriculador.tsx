...
import PopAddAdmin from "../Administrador/PopAddAdmin";
const Matriculador = () => {
  const wsM = useMemo(() => {
  ...
  useEffect(() => {
    ...
    wsM.onmessage = (event) => {
      ...
      if (data.action === "VIEW_ALL") {
        setAdmins(
          data.view.filter((user: Usuario) => user.rol === 4) as Usuario[]
        );
      }
  ...
  const addMat = (id: string, nombre: string, clave: string) =>
    wsM.send(
      JSON.stringify({
        action: "CREATE",
        id: parseInt(id),
  ...
  const editMat = (id: string, nombre: string, clave: string) =>
    wsM.send(
      JSON.stringify({
        action: "UPDATE",
        id: parseInt(id),
                  ...
                  <>
                    <PopoverTrigger>
                      <Button colorScheme="teal" variant="ghost">
                        Click aca para agregar un matriculador
                      </Button>
                    </PopoverTrigger>
                    <Portal>
                      {isOpen && (
